...
describe("admin approval API", () => {
  ...
  it("the whole lifecycle: register PENDING -> can't log in -> approved -> can log in", async () => {
    ...
    const pending = await api.get("/api/v1/auth/users?status=PENDING").set("Authorization", `Bearer ${superToken}`);
    expect(pending.status).toBe(200);
    expect(pending.body.users.map((u: { id: string }) => u.id)).toContain(reg.body.id);

    const approve = await api
      .post(`/api/v1/auth/users/${reg.body.id}/approve`)
  ...
  it("reject sets the account DISABLED", async () => {
    ...
    const res = await api
      .post(`/api/v1/auth/users/${reg.body.id}/reject`)
      .set("Authorization", `Bearer ${superToken}`);
  ...
  it("requires an ADMIN / SUPER_ADMIN token", async () => {
    ...
    expect((await api.get("/api/v1/auth/users?status=PENDING")).status).toBe(401);
    expect(
      (await api.get("/api/v1/auth/users?status=PENDING").set("Authorization", `Bearer ${patientToken}`)).status,
    ).toBe(403);
  });
  ...
  it("an ADMIN cannot see or approve another ADMIN - only a SUPER_ADMIN can", async () => {
    ...
    await api.post(`/api/v1/auth/users/${admin1.body.id}/approve`).set("Authorization", `Bearer ${superToken}`);
    const admin1Token = (
    ...
    const queue = await api.get("/api/v1/auth/users?status=PENDING").set("Authorization", `Bearer ${admin1Token}`);
    expect(queue.body.users.map((u: { id: string }) => u.id)).not.toContain(admin2.body.id);

    // ...and a direct approve attempt is forbidden.
    const attempt = await api
      .post(`/api/v1/auth/users/${admin2.body.id}/approve`)
  ...
  it("approving a non-PENDING user is a 400", async () => {
    ...
    await api.post(`/api/v1/auth/users/${reg.body.id}/approve`).set("Authorization", `Bearer ${superToken}`);
  ...
  it("approving an unknown user id is a 404", async () => {
    const superToken = (await activeUserAccessToken({ clientId: "superadmin-portal" })).accessToken;
    const res = await api
      .post(`/api/v1/auth/users/00000000-0000-0000-0000-000000000000/approve`)
      .set("Authorization", `Bearer ${superToken}`);

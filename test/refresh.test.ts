...
describe("POST /api/v1/auth/token/refresh - native (body) transport", () => {
  beforeEach(resetDb);
  ...
  it("rotates: the old token stops working, the new one works", async () => {
    ...
    const r1 = await api.post("/api/v1/auth/token/refresh").send({ refreshToken: s.refreshToken });
    ...
    const reused = await api.post("/api/v1/auth/token/refresh").send({ refreshToken: s.refreshToken });
    ...
    const afterReuse = await api.post("/api/v1/auth/token/refresh").send({ refreshToken: r1.body.refreshToken });
    expect(afterReuse.status).toBe(403);
  });

  it("rejects a garbage token with 401", async () => {
    const res = await api.post("/api/v1/auth/token/refresh").send({ refreshToken: "not-a-real-token" });
    expect(res.status).toBe(401);
    expect(res.body.error.code).toBe("NOT_AUTHENTICATED");
  });

  it("rejects a missing token with 401", async () => {
    const res = await api.post("/api/v1/auth/token/refresh").send({});
    expect(res.status).toBe(401);
  });
  ...
  it("logout revokes the session; the token can no longer refresh", async () => {
    ...
    const out = await api.post("/api/v1/auth/logout").send({ refreshToken: s.refreshToken });
    expect(out.status).toBe(204);

    const res = await api.post("/api/v1/auth/token/refresh").send({ refreshToken: s.refreshToken });
  ...
  it("logout-all revokes every session for the user", async () => {
    ...
    const res = await api.post("/api/v1/auth/logout-all").set("Authorization", `Bearer ${a.accessToken}`);
    expect(res.status).toBe(200);
    expect(res.body.revokedSessions).toBe(2);

    for (const token of [a.refreshToken, b.refreshToken]) {
      expect((await api.post("/api/v1/auth/token/refresh").send({ refreshToken: token })).status).toBe(403);
    }
  });

  it("logout-all requires a valid access token", async () => {
    expect((await api.post("/api/v1/auth/logout-all")).status).toBe(401);
    expect(
      (await api.post("/api/v1/auth/logout-all").set("Authorization", "Bearer garbage")).status,
    ).toBe(401);
  });
});

describe("POST /api/v1/auth/token/refresh - web (cookie) transport", () => {
  beforeEach(resetDb);
  ...
  it("rotates via the httpOnly cookie and detects reuse of the old cookie", async () => {
    ...
    const li = await api.post("/api/v1/auth/email/login").send({ email, password: PASSWORD, clientId: "patient-portal" });
    const first = readSetCookie(li, "refresh_token")!;
    expect(first).toBeTruthy();

    const r1 = await api.post("/api/v1/auth/token/refresh").set("Cookie", `refresh_token=${first}`);
    expect(r1.status).toBe(200);
    expect(r1.body.accessToken).toBeTruthy();
    ...
    const reused = await api.post("/api/v1/auth/token/refresh").set("Cookie", `refresh_token=${first}`);
    expect(reused.status).toBe(403);
  });
});
